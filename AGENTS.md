...
1. The five bundled skills describe **Aurelia v2 only**. The seven v1 APIs — `.delegate` (on custom events), `<router-view>`, `PLATFORM.moduleName`, `configureRouter`, `<compose>`, `@inject`, `activate/deactivate` — are pinned as prohibitions in every pillar. Never soften a prohibition in `skills/*/SKILL.md` or `reference/*.md`.
...
3. The router (`aurelia-expert`) is the **single entry point**. It hands off to a pillar and stops. The four pillars are peer-skills, not nested; never re-route from one pillar to another. If two branches both fit, defer to the more specific one (`aurelia-migration` > `aurelia-largespa` for migration-of-large-SPA prompts; see `aurelia-expert/SKILL.md` Precedence section).
...
<task>Bundle five router-routed markdown skills into a publishable OpenCode plugin + npm package</task>
<execution>npm install `aurelia-expert` → OpenCode loads `plugin.ts#config()` → idempotent install of five skills into `.opencode/skills/`</execution>
</context_hierarchy>
...
<purpose>Register `../skills` as a skill path and pre-allow all five skills for the package's own dev workflow</purpose>
